// This file contains the initial implementations of the NN's layers.
...
#pragma HLS INLINE off
	...
	for (int p = 0 ; p<DENSE3_NEURONS; p++) {    
		...
		read_value = 1+output[p]; //first order Taylor of exp
...
#pragma HLS INLINE off
	...
	for (int p = 0; p < CONV1_NEURONS; p ++) { 
        ...
        //Inserting the biases
        conv_layer1_biases_outer_loop:
        for ( int row = 0; row < H-IS_VALID; row++ ) {
        	conv_layer1_biases_inner_loop:
            for ( int col = 0; col < W-IS_VALID; col++ ) {
                output[p][row][col] = (CL1_out_t) conv_1_biases[p]; 
            }
        }
...
#pragma HLS INLINE off
	...
	for (int p = 0; p < CONV2_NEURONS; p ++) {
        ...
        //Inserting the biases
        conv_layer2_biases_outer_loop:
		for ( int row = 0; row < H-IS_VALID; row++ ) {
			conv_layer2_biases_inner_loop:
            for ( int col = 0; col < W-IS_VALID; col++ ) {
                output[p][row][col] = (CL2_out_t) conv_2_biases[p]; 
            }
        }
